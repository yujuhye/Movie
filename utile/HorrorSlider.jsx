...
const CurSlider = () => {
    ...
    setData(Object.values(movieData));
        ...
        const renderMovieLinks = () => {
            return (
                data?.length ? (
                    data.slice(25, 31).map((item, index) => ( 
                        <div className="genreContainer" key={index + 26} style={{width:'180px', height:'400px'}}>
                            <li className="listContainer" style={imageSize}>
                                <Link className="genreList" to={`/movieInfo/${index + 26}`}>
                                    <img
                                        className="gImg"
                                        style={imgStyle}
                                        src={item.poster}
                                        alt={`포스터 이미지 ${index + 26}`}
                                    />
