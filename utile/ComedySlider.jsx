...
const CurSlider = () => {
    ...
    setData(Object.values(movieData));
        ...
        const renderMovieLinks = () => {
            return (
                data?.length ? (
                    data.slice(20, 26).map((item, index) => ( 
                        <div className="genreContainer" key={index + 21} style={{width:'180px', height:'400px'}}>
                            <li className="listContainer" style={imageSize}>
                                <Link className="genreList" to={`/movieInfo/${index + 21}`}>
                                    <img
                                        className="gImg"
                                        style={imgStyle}
                                        src={item.poster}
                                        alt={`포스터 이미지 ${index + 21}`}
                                    />
